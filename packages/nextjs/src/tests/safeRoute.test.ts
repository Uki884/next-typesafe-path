...
describe('safeRoute', () => {
  ...
  describe('routes with only search params', () => {
    test('required search params', () => {
      expect(safeRoute('/shop/', { isRequired: true, tags: ['a', 'b'] }))
        .toBe('/shop/?isRequired=true&tags=a&tags=b');
    });

    test('optional search params', () => {
      expect(safeRoute('/login/', { redirect: '/dashboard' })).toBe('/login/?redirect=%2Fdashboard');
  ...
  describe('routes with only dynamic params', () => {
    ...
    test('optional params (catch-all)', () => {
      expect(safeRoute('/products/[[...filters]]/', { filters: [] }, { page: 1 })).toBe('/products/?page=1');
      expect(safeRoute('/products/[[...filters]]/', { filters: ['sale', 'winter'] }, { page: 1 }))
        .toBe('/products/sale/winter/?page=1');
    });

    test('required params (catch-all)', () => {
      expect(safeRoute('/shop/[...categories]/', { categories: ['men', 'shoes'] }, { page: 1 }))
  ...
  describe('routes with both params and search params', () => {
    ...
    test('optional params and required search params', () => {
      expect(safeRoute('/products/[[...filters]]/', { filters: [] }, { page: 1 }))
    ...
    test('optional params and optional search params', () => {
      ...
      expect(safeRoute('/products/[[...filters]]/', { filters: ['sale'] }, { page: 1 }))
        .toBe('/products/sale/?page=1');
      expect(safeRoute('/products/[[...filters]]/', { filters: ['sale'] }, { sort: 'asc', page: 1 }))
  ...
  test('multiple dynamic params', () => {
    expect(safeRoute('/users/[userId]/posts/[postId]/', {
      userId: '123',
      postId: '456'
    }, { page: 1 })).toBe('/users/123/posts/456/?page=1');
  });
  ...
  test('handles special characters and encoding', () => {
    ...
    expect(safeRoute('/users/[id]/', { id: '123+456' }, { tab: 'profile', special: '&special=' }))
      .toBe('/users/123+456/?tab=profile&special=%26special%3D');
  });

  // Case 7: Array parameters
  test('handles array parameters', () => {
    expect(safeRoute('/shop/', { tags: ['t1', 't2'], isRequired: true }))
      .toBe("/shop/?tags=t1&tags=t2&isRequired=true");
  });
